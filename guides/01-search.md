...
piece looks the way it does. The code already exists — read it alongside this.
...
— not part of our domain. Two `serde` attributes carry the load:
...
feeds it a saved fixture and asserts on the result — fast and deterministic.

`.into_iter()` (not `.iter()`) consumes `response`, so we *move* each `id` and
`title` `String` out instead of cloning. Since we own `response` and never use it
again, moving is free. Reach for `into_iter` when you're done with the source.
...
Search works without one; the key just lifts Google's anonymous rate limit, so it's
pushed in only `if let Some(key)`.

The method does I/O and *nothing else clever* — it fetches text and hands it to the
...
- `.error_for_status()?` turns an HTTP 4xx/5xx into a `reqwest::Error`, which `?`
  ...
  were a book list.
- `reqwest::blocking` — we use the *blocking* client on purpose. A one-shot CLI makes
...
The two parameters that make this testable:
...
`writeln!(...)?` can fail (a closed pipe, e.g. `book search x | head`), and that `?`
turns it into `BookError::Io`. Propagating write errors instead of ignoring them is
why `test_search_propagates_write_errors` exists.

The tab-separated `id\ttitle` output is intentional: it's pipe-friendly
(`book search rust | cut -f1`).
...
`main` is the only place that picks the *real* implementations. It locks stdout once
(faster than re-locking per line) and hands both to the handler. Notice `main` has no
logic — just construction and delegation. That's the goal.
...
`FakeSearch` returns canned hits and ignores the query. That's enough to test the
handler's *behavior* (does it print id and title? does it propagate write errors?)
without a single packet leaving the machine. `fetch` is `unimplemented!()` here
because branch 1 never calls it — it gets a real fake in branch 2.

The `FailingWriter` test double is the other half: a `Write` whose `write()` always
returns `BrokenPipe`, proving the handler surfaces I/O errors rather than swallowing
them.
